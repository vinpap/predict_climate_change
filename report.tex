...
\begin{document}
    ...
    Cette section couvre la mise en place d'un modèle SARIMA, de l'entraînement du modèle à sa mise à disposition sur une API. Le code relatif à cette partie du rapport est disponible dans les \hyperref[sec:code]{annexes}.

    \subsection{Chargement des données et analyses préliminaires}

    La première étape consiste à charger nos données. Pour cela, nous utiliserons \textbf{pandas} pour lire le CSV qui contient nos données et enregistrer celles qui nous intéressent dans une dataframe. [ET POUR LES VALEURS MANQUANTES ?]
