...
try:
    ...
    print("NOTE: DB_PASSWORD has not been set in the environment variables. This is only a problem if you are loading monitoring data from a PostgreSQL database")
...
def parse_args():
    ...
    parser.add_argument(
        "--setup", "-s", action="store_true", help="Trains the model through the API for the first time and defines the performance threshold"
    )
...
def load_from_csv(filepath: str) -> pd.DataFrame:
    ...
    filtered_data["measurement_dates"] = pd.to_datetime(filtered_data["measurement_dates"])
    filtered_data["measurement_dates"] = filtered_data["measurement_dates"].dt.strftime('%m/%Y')
...
def send_email(subject: str, body: str):
    ...
    with open("./monitoring_cfg.yml", "r") as config_file:
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp_server:
            ...
            smtp_server.sendmail("papelardvincent@gmail.com", recipient, msg.as_string())
            print("Message sent!")
...
def setup():
    ...
    with open("./monitoring_cfg.yml", "r") as config_file:
        ...
        request_data = {"start_date": train["measurement_dates"].iloc[0],
        ...
        if response.status_code < 200 or response.status_code >= 300:
            raise RuntimeError("Unable to use the API, please make sure it is up and running at the URL specified in monitoring_cfg.yml")

        # Testing the model and saving 105% of the RMSE as the new performance threshold
        test_request_data = {"start_date": test["measurement_dates"].iloc[0],
                         "temperatures": list(test["temperatures"]),
                         "secret_token": API_TOKEN}

        response = requests.post(url=cfg["API_ENDPOINT"] + "test", json=test_request_data)
...
def monitor():
    ...
    with open("./monitoring_cfg.yml") as config_file:
        ...
        if "RMSE_THRESHOLD" not in cfg or cfg["RMSE_THRESHOLD"] == -1:
            print("No performance threshold found in the configuration file. Please run this script with the --setup flag at least once before using it to monitor the prediction model")
            sys.exit(0)
        data = load_data(cfg)
        threshold = cfg["RMSE_THRESHOLD"]

        test_request_data = {"start_date": data["measurement_dates"].iloc[0],
        ...
        if rmse > threshold:
            print(f"The model performance went above the root mean squared error set at {threshold}. Now retraining the model")
            ...
            if rmse > threshold:
                print(f"RMSE is still too high after retraining the model ({rmse} compared to a threshold set at {threshold}). Sending an email alert to {cfg['EMAIL_SETTINGS']['EMAIL_RECIPIENT']}")
