...
async def train(data: WeatherData):
    ...
    try:
        ...
        return {
            "message": "Please provide the temperature series start date in format mm/yyyy"
        }
...
async def predict(forecast_date: ForecastDate):
    ...
    if not model:
        return {
            "message": "No model has been trained. Please train the model first by sending monthly temperature series to the /train endpoint"
        }
    ...
    if forecast_date <= model_dates[-1]:
        return {
            "message": f"The date you entered is older than the last date stored by the forecast model, which is {str(model_dates[-1])}. Please enter a date after this one."
        }
...
async def test(data: WeatherData):
    ...
    if start_date <= model_dates[-1]:
        ...
        if len(test_temp) <= months_diff:
            return {
                "message": "The test data you sent does not include any unknown data, therefore it is impossible to test the model with it"
            }
